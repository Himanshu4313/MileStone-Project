...
const displayBlog = () => {
  ...
  blog.forEach((obj, idx) => {
    element += `<div id="card">
        ...
        <h3>Blog title : ${obj.blogtitle}</h3>
...
//form input target element
const postURL = document.getElementById("PostURL");
const blogTitleInput = document.getElementById("BlogTitleInput");
const blogDescriptionInput = document.getElementById("BlogDescriptionInput");
const blogFullDescription = document.getElementById("BlogFullDescription");
const addBlogFormBtn = document.getElementById("AddBlogBtn");

function FillBlogInput() {
  let posturl = postURL.value;
  let blogtitleinput = blogTitleInput.value;
  let blogdescriptioninput = blogDescriptionInput.value;
  let blogfulldescription = blogFullDescription.value;

  const obj = {
    image: `${posturl}`,
    blogtitle: `${blogtitleinput}`,
    blogdescription: `${blogdescriptioninput}`,
    readmore: `${blogfulldescription}`,
  };
  blog.push(obj);
  // console.log(blog)
...
addBlogFormBtn.addEventListener("click", () => {
  if (postURL.value === "") {
    alert("Please Fill This Blog Form");
  } else {
    FillBlogInput();
  }
});
